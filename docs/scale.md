...
| `--concurrency` | Max in-flight rows. Capped by `READYAGENTS_MAX_CONCURRENCY` (hard ceiling; a workflow cannot raise it). |
...
starve a foreground run. Provider `Retry-After` pauses new work for that
provider (back-pressure, not a retry storm).

`READYAGENTS_MAX_CONCURRENCY` is the hard ceiling (default 4096, clamped).
Workflow YAML cannot raise it.
