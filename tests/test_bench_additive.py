"""Byte-identical no-bench guard for V2-16. Written before bench lands."""
...
def test_calc_pipeline_without_bench_json_keys() -> None:
    first = runner.invoke(
    ...
    blob = json.dumps(a)
    assert "bench" not in blob
    assert a["command"] == b["command"] == "run"


def test_eval_pass_without_bench() -> None:
