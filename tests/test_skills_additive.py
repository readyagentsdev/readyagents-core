"""Byte-identical no-skill guard for V2-03. Written before type: skill lands."""
...
def test_calc_pipeline_without_skill_json_keys() -> None:
    first = runner.invoke(
    ...
    blob = json.dumps(a)
    assert "type: skill" not in blob
    assert '"skill"' not in json.dumps(a.get("node_results") or [])


def test_approval_gate_without_skill_still_pauses(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "type: skill" not in json.dumps(done)
