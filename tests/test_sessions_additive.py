"""Byte-identical no-session guard for V2-25. Written before type: converse lands."""
...
def test_calc_pipeline_without_session_json_keys() -> None:
    first = runner.invoke(
    ...
    assert '"converse"' not in blob
    assert a["status"] == "succeeded"


def test_eval_pass_without_session() -> None:
...
def test_core_dependencies_have_no_audio_engine() -> None:
    text = (_root() / "pyproject.toml").read_text(encoding="utf-8")
    core = text.split("[project.optional-dependencies]", 1)[0].lower()
    for needle in ("webrtc", "sip", "pyaudio", "sounddevice", "opentelephony"):
...
def test_calc_and_approval_examples_clean() -> None:
    ...
    for name in names:
        ...
        assert "type: converse" not in text
