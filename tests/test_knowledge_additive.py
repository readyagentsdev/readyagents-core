"""Byte-identical no-ingest guard for V2-11. Written before type: ingest lands."""
...
def test_calc_pipeline_without_ingest_json_keys() -> None:
    first = runner.invoke(
    ...
    blob = json.dumps(a)
    assert "type: ingest" not in blob
    assert "document_id" not in blob


def test_approval_gate_without_ingest_still_pauses(tmp_path: Path, monkeypatch) -> None:
...
def test_memory_triage_write_search_unchanged(tmp_path: Path, monkeypatch) -> None:
    clear_settings_cache()
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("READYAGENTS_HOME", str(tmp_path / ".readyagents"))
    example = _root() / "examples" / "memory_triage.yaml"
    first = runner.invoke(app, ["run", str(example), "--json", "--no-persist"])
    assert first.exit_code == 0, first.stdout + first.stderr
    data = _payload(first.stdout)
    assert data["ok"] is True
    assert data["status"] == "succeeded"
    prior = data["output_keys"]["prior"]
    assert "hits" in prior
    assert prior.get("retrieval") == "keyword"
    assert "blend" not in prior
    clear_settings_cache()
...
def test_calc_and_approval_examples_clean() -> None:
    ...
    for name in names:
        ...
        assert "type: ingest" not in text
