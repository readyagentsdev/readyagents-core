"""Byte-identical no-feedback guard for V2-15. Written before feedback lands."""
...
def test_calc_pipeline_without_feedback_json_keys() -> None:
    first = runner.invoke(
...
def test_eval_pass_without_feedback() -> None:
...
def test_approval_gate_without_feedback_still_pauses(tmp_path: Path, monkeypatch) -> None:
    ...
    resumed = runner.invoke(app, ["resume", data["run_id"], "--approve", "gate", "--json"])
    assert resumed.exit_code == 0, resumed.stdout + resumed.stderr
    done = _payload(resumed.stdout)
    assert done["ok"] is True
    assert done["status"] == "succeeded"
    assert "feedback" not in json.dumps(done.get("metadata") or {})
    clear_settings_cache()
