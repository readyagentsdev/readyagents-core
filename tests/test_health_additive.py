"""Byte-identical no-recovery guard for V2-20. Written before health lands."""
...
def test_calc_pipeline_without_recovery_json_keys() -> None:
    first = runner.invoke(
    ...
    assert "quarantine" not in blob
    assert a["command"] == b["command"] == "run"


def test_eval_pass_without_health() -> None:
