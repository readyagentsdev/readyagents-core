"""Byte-identical no-wait guard for V2-13. Written before type: wait lands."""
...
def test_calc_pipeline_without_wait_json_keys() -> None:
    first = runner.invoke(
    ...
    blob = json.dumps(a)
    assert "waiting" not in blob
    assert "type: wait" not in blob
    assert a["status"] != "waiting"


def test_approval_gate_without_wait_still_pauses(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "waiting" not in json.dumps(done)
